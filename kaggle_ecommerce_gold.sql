...
-- ==================================================================================================
--                              ARMAZENAR TABELAS NA CAMADA GOLD
-- ================================================================================================== 
-- CREATE TABLE kaggle_gold.XXXXXXXXX AS  
-- Não funciona com Format - Tive que excluir.
-  --------------------------------------  TOTAL DE VENDA X ANO  --------------------------------------------- 
CREATE TABLE kaggle_gold.total_vendas_2016 AS  
SELECT FORMAT(SUM(price),2) as TOTAL_VENDAS_2016
FROM itens_silver
WHERE YEAR(shipping_limit_date) = 2016;
-- R$ 49.785,92
CREATE TABLE kaggle_gold.total_vendas_2017 AS  
SELECT FORMAT(SUM(price),2) as TOTAL_VENDAS_2016
FROM itens_silver
WHERE YEAR(shipping_limit_date) = 2017;
-- R$ 6.034.868,60 
CREATE TABLE kaggle_gold.total_vendas_2018 AS  
SELECT FORMAT(SUM(price),2) as TOTAL_VENDAS_2016
FROM itens_silver
...
CREATE TABLE kaggle_gold.vendas_por_vendedor_ano AS  
...
CREATE TABLE kaggle_gold.avalicoes_vendedores AS  
...
Group By v.seller_id
Having QUANT_VENDAS > 1000 AND QUANT_VENDAS < 2000;

--  -------------------------------   PORCENTAGEM DE REINCIDÊNCIA -------------------------------  
CREATE TABLE kaggle_gold.reincidencia AS  
SELECT 
...
CREATE TABLE kaggle_gold.vendas_x_avaliacoes_vendedores AS  
Select v.seller_id as vendedor, 
...
CREATE TABLE kaggle_gold.dados_vendedor_selecionado AS  
...
--  ---------------------------------  MAIORES VENDEDORES ---------------------------------------- 
CREATE TABLE kaggle_gold.maiores_vendedores AS 
SELECT v.seller_id as Vendedor,
...
CREATE TABLE kaggle_gold.vendas_categoria AS 
...
CREATE TABLE kaggle_gold.categorias_adicionadas2016 AS 
SELECT p.product_category_name AS ADICIONADOS_EM_2016,
	SUM(CASE WHEN YEAR(i.shipping_limit_date) = 2016 THEN i.price ELSE 0 END) AS R$VENDAS2016
FROM produtos_silver as p
...
CREATE TABLE kaggle_gold.categorias_adicionadas2017 AS 
SELECT p.product_category_name AS ADICIONADOS_EM_2017,
	SUM(CASE WHEN YEAR(i.shipping_limit_date) = 2016 THEN i.price ELSE 0 END) AS R$VENDAS2016,
    SUM(CASE WHEN YEAR(i.shipping_limit_date) = 2017 THEN i.price ELSE 0 END) AS R$VENDAS2017
FROM produtos_silver as p
LEFT JOIN itens_silver as i ON i.product_id = p.product_id
...
CREATE TABLE kaggle_gold.categorias_adicionadas2018 AS 
SELECT p.product_category_name AS ADICIONADOS_EM_2018,
	SUM(CASE WHEN YEAR(i.shipping_limit_date) = 2016 THEN i.price ELSE 0 END) AS R$VENDAS2016,
    SUM(CASE WHEN YEAR(i.shipping_limit_date) = 2017 THEN i.price ELSE 0 END) AS R$VENDAS2017,
    SUM(CASE WHEN YEAR(i.shipping_limit_date) = 2018 THEN i.price ELSE 0 END) AS R$VENDAS2018
FROM produtos_silver as p
JOIN itens_silver as i ON i.product_id = p.product_id
...
CREATE TABLE kaggle_gold.vendas_anuais_estado AS 
Select c.customer_state as ESTADO, 
	p.product_category_name as CATEGORIAS_MAIS_VENDIDA,
	SUM(i.price) AS R$TOTALVENDAS,
    AVG(i.price) AS TICKET_MÉDIO
FROM produtos_silver as p
LEFT Join itens_silver as i ON i.product_id = p.product_id
LEFT Join pedidos_silver as pe ON pe.order_id = i.order_id
LEFT Join clientes_silver as c ON c.customer_id = pe.customer_id
...
CREATE TABLE vendas_com_reincidência AS 
...
CREATE TABLE kaggle_gold.analise_reincidencia AS
